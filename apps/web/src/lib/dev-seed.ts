...
export async function ensureDevLoginUser(headers = new Headers()) {
  ...
  if (existing) {
    ...
    throw new Error(
      `User ${existing.id} has no credential account; aborting dev login to avoid data loss.`
    )
  }
...
export async function seedDevWorkspace() {
  ...
  return db.transaction(async (tx) => {
    ...
    const [existingWorkflow] = await tx
      ...
      .where(
        and(
          eq(workflows.repoId, repo.id),
          eq(workflows.name, "Review new contributors")
        )
      )
      .limit(1)
    ...
    if (existingWorkflow) {
      await tx
        ...
        .set({
          description:
            "Warn when a new pull request comes from a young account.",
    ...
    const [existingCustomRule] = await tx
      .select({ id: customRules.id })
      .from(customRules)
      .where(
        and(
          eq(customRules.repoId, repo.id),
          eq(customRules.name, "Urgent title check")
        )
      )
      .limit(1)
